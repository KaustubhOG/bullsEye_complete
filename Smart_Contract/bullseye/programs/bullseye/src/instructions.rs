...
use crate::state::*;
use crate::errors::ErrorCode as BullseyeError;
...
pub fn initialize_goal(
    ...
    // Validations
    require!(title.len() <= 100, BullseyeError::TitleTooLong);
    require!(description.len() <= 500, BullseyeError::DescriptionTooLong);
    require!(amount >= 100_000_000, BullseyeError::AmountTooLow); // 0.1 SOL
    require!(amount <= 10_000_000_000, BullseyeError::AmountTooHigh); // 10 SOL
    
    let clock = Clock::get()?;
    require!(deadline > clock.unix_timestamp, BullseyeError::DeadlineInPast);
    ...
    if let Some(_active_goal_number) = goal_counter.active_goal {
        return Err(BullseyeError::ActiveGoalExists.into());
    }
...
pub fn submit_for_verification(ctx: Context<SubmitForVerification>) -> Result<()> {
    ...
    require!(goal.status == GoalStatus::Active, BullseyeError::InvalidGoalStatus);
    require!(ctx.accounts.user.key() == goal.user, BullseyeError::Unauthorized);
...
pub fn cast_vote(ctx: Context<CastVote>, vote: bool) -> Result<()> {
    ...
    require!(goal.status == GoalStatus::Submitted, BullseyeError::InvalidGoalStatus);
    require!(!verification.finalized, BullseyeError::AlreadyFinalized);
    ...
    let verifier_index = verification
        ...
        .ok_or(BullseyeError::NotAVerifier)?;

    // Check if already voted
    require!(!verification.votes_cast[verifier_index], BullseyeError::AlreadyVoted);
...
pub fn finalize_verification(ctx: Context<FinalizeVerification>) -> Result<()> {
    ...
    require!(
        ...
        BullseyeError::VerificationNotComplete
    );
...
pub fn claim_or_distribute(ctx: Context<ClaimOrDistribute>) -> Result<()> {
    ...
    require!(goal.status == GoalStatus::Submitted, BullseyeError::InvalidGoalStatus);
    require!(verification.finalized, BullseyeError::VerificationNotFinalized);
    ...
    let result = if deadline_passed {
        ...
        verification.result.ok_or(BullseyeError::NoVerificationResult)?
    };
